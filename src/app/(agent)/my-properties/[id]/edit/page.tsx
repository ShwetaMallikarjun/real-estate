...
export default function AgentEditPropertyPage({
  ...
  useEffect(() => {
    async function load() {
      try {
        const [propertyRes, locationsRes, developersRes] = await Promise.all([
  ...
  async function onSubmit(data: PropertyInput) {
    ...
    const gallery = galleryInput.split(",").map((s) => s.trim()).filter(Boolean);
    const videoReview = videoInput.split(",").map((s) => s.trim()).filter(Boolean);
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-500">Ubah data properti Anda</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <Card>
          ...
          <CardContent className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Input id="harga_asli" type="number" {...register("harga_asli", { valueAsNumber: true })} />
            ...
            <div className="space-y-2">
              ...
              <Input id="harga_diskon" type="number" {...register("harga_diskon", { valueAsNumber: true })} />
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Textarea id="gallery" rows={3} value={galleryInput} onChange={(e) => setGalleryInput(e.target.value)} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="videoReview">Video Review YouTube (URL dipisah koma)</Label>
              <Textarea id="videoReview" rows={3} value={videoInput} onChange={(e) => setVideoInput(e.target.value)} />
            </div>
          </CardContent>
        </Card>
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button type="submit" disabled={isSubmitting}>
            {isSubmitting ? (<><Spinner size="sm" /> Menyimpan...</>) : "Update Properti"}
          </Button>
        </div>
      </form>
    </div>
